...
(* Successively more explicit printing flags: coercions, implicit
   arguments, sorts, sorts with anonymous quality variables, no
   notations, universes, parentheses, ending with the equivalent of
   ...
   Printing sort quality variables anonymously (as [_], which re-parses
   to a fresh quality variable, rather than their raw unparsable
   [α]-names) is tried right after plain sorts, and each universes rung
   is tried with anonymous quality variables first and then without: a
   fresh quality variable printed by its raw name cannot be re-parsed,
   but universe printing may make the anonymous form ambiguous, so we
   fall back to the non-anonymous form. Raw printing forces the flag
   off, so the final rung stays faithful.

   Only extern/detype-level flags and the [parentheses] flag may vary
   along the ladder: the rendering of the returned [constr_expr] must be
