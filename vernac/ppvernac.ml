...
let pr_cumul_univ_decl_qualities l extensible =
  (* "extensible" not really supported in syntax currently *)
  if List.is_empty l then mt()
...
let pr_cumul_univ_decl l =
  ...
  | Some l ->
    str"@{" ++
    pr_cumul_univ_decl_qualities l.univdecl_qualities l.univdecl_extensible_qualities ++
...
let pr_synpure_vernac_expr v =
  ...
  | VernacSchemeAll (id, lid) ->
    return (
      hov 2 (keyword "Scheme " ++ keyword "All" ++ str " for" ++ spc() ++ pr_smart_global id
              ++ pr_opt (fun x -> str " over" ++ spc() ++ prlist_with_sep pr_comma Id.print x) lid)
    )
  | VernacScheme l ->
    return (
