...
:-vok: Indicate Rocq to check a file completely, to load ``.vos`` files instead
  of ``.vo`` files when interpreting :cmd:`Require` commands. No ``.vo`` file is written.
...
correctly compiles, including all its opaque proofs.
...
Empty `.vos` and `.vok` files are created by the `.vo` targets of
`rocq makefile`, and an empty `.vok` by the `.vok` targets of `rocq
makefile`.
...
When compiling a file ``foo.v`` using ``rocq compile`` invoked through
the makefile generated by `rocq makefile` for a `.vo` target, an empty
file ``foo.vos`` and an empty file ``foo.vok`` are created in addition
to the regular output file ``foo.vo``. If ``rocq compile`` is
subsequently invoked on some other file ``bar.v`` using option
``-vos`` or ``-vok``, and that ``bar.v`` requires ``foo.v``, if Rocq
finds an empty file ``foo.vos``, then it will load ``foo.vo`` instead
of ``foo.vos``.
