...
let extern_glob_qvar ~anon uvars = function
  ...
  | GQVar q -> begin match UnivNames.qualid_of_quality uvars q with
    ...
    | None ->
      (* A quality variable with no name has no parsable form; print it
         ...
         [anon]. *)
      if anon then CQAnon None else CRawQVar q
    end

let extern_relevance ~anon uvars = function
  | GRelevant -> CRelevant
  | GIrrelevant -> CIrrelevant
  | GRelevanceVar q -> CRelevanceVar (extern_glob_qvar ~anon uvars q)
...
let extern_glob_quality ~anon uvars = function
  ...
  | GQualVar q -> CQualVar (extern_glob_qvar ~anon uvars q)

let extern_glob_sort ~anon uvars (q, l) =
  Option.map (extern_glob_qvar ~anon uvars) q,
