...
| QSort (q1, u1), s2 ->
  ...
  let qcsts = UVars.QUnifConstraints.add (QVar q1, Eq, q2) qcsts in
...
| s1, QSort (q2, u2) ->
  ...
  let qcsts = UVars.QUnifConstraints.add (q1, Eq, QVar q2) qcsts in
...
| QSort (q1, u1), s2 ->
  let q2 = quality s2 in
  let qcsts = UVars.QUnifConstraints.singleton (QVar q1, Le, q2) in
...
| s1, QSort (q2, u2) ->
  let q1 = quality s1 in
  let qcsts = UVars.QUnifConstraints.singleton (q1, Le, QVar q2) in
  ...
  (qcsts, ucsts), g
| (SProp, SProp) | (Prop, Prop) | (Set, Set) -> (UVars.QUnifConstraints.empty, Univ.UnivConstraints.empty), g
| (Prop, (Set | Type _)) -> (UVars.QUnifConstraints.empty, Univ.UnivConstraints.empty), g
| (((Prop | Set | Type _) as s1), (Prop | SProp as s2))
...
| (Set | Type _), (Set | Type _) ->
  ...
  (UVars.QUnifConstraints.empty, ucsts), g
...
let infer_eq (univs, cstrs as cuniv) s s' =
  if UGraph.check_eq_sort Sorts.Quality.equal univs s s' then Result.Ok cuniv
    ...
    let qcsts', ucstrs' as cstrs' = enforce_eq_sort s s' (UVars.QUnifConstraints.empty, Univ.UnivConstraints.empty) in
